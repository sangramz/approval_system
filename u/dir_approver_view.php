...
function ViewPendApprov($view) {
  ...
  switch ($view) {
    ...
    case 'money_requisition':
      require_once 'db.php';
      $stmt = $dbcon->prepare("SELECT * FROM mr WHERE direct_approv='FALSE'AND hod_approv='1'");
      $stmt->execute();
      ...
      case 'bills_proc':
        require_once 'db.php';
        $stmt = $dbcon->prepare("SELECT * FROM accounts_bl WHERE bl_director_approv='0' AND bl_hod_approv='1'");
        $stmt->execute();
        $rowCount = $stmt->fetchAll();
        $nRows = count($rowCount);
            if ($nRows > 0) {
        ...
        case 'vendor_payment':
          require_once 'db.php';
          $stmt = $dbcon->prepare("SELECT * FROM accounts_vp WHERE vp_director_approv='FALSE' AND vp_hod_approv='1'");
          $stmt->execute();
          $rowCount = $stmt->fetchAll();
          $nRows = count($rowCount);
              if ($nRows > 0) {
          ...
          case 'daily_sales':
            require_once 'db.php';
            $stmt = $dbcon->prepare("SELECT * FROM sales_dscr WHERE dscr_director_approv ='FALSE' AND dscr_hod_approv='1'");
            $stmt->execute();
            $rowCount = $stmt->fetchAll();
            $nRows = count($rowCount);
                if ($nRows > 0) {
            ...
            case 'transport':
              require_once 'db.php';
              $stmt = $dbcon->prepare("SELECT * FROM sales_transport WHERE transport_director_approv ='FALSE'AND transport_hod_approv='TRUE'");
              $stmt->execute();
              $rowCount = $stmt->fetchAll();
              $nRows = count($rowCount);
                  if ($nRows > 0) {
              ...
              case 'site':
                ...
                case 'recruitment':
                  require_once 'db.php';
                  $stmt = $dbcon->prepare("SELECT * FROM hr_rec WHERE rec_director_approv ='FALSE' AND rec_hod_approv='1'");
                  $stmt->execute();
                  $rowCount = $stmt->fetchAll();
                  $nRows = count($rowCount);
                      if ($nRows > 0) {
                  ...
                  case 'leave':
                    require_once 'db.php';
                    $stmt = $dbcon->prepare("SELECT * FROM hr_lev WHERE lev_director_approv ='FALSE' AND lev_hod_approv='1'");
                    $stmt->execute();
                    $rowCount = $stmt->fetchAll();
                    $nRows = count($rowCount);
                        if ($nRows > 0) {
                    ...
                    case 'vendor_registeration':
                      require_once 'db.php';
                      $stmt = $dbcon->prepare("SELECT * FROM ven_reg WHERE ven_director_approv ='FALSE' AND ven_hod_approv='1'");
                      $stmt->execute();
                      $rowCount = $stmt->fetchAll();
                      $nRows = count($rowCount);
                          if ($nRows > 0) {
                      ...
                      case 'po_approvals':
                        require_once 'db.php';
                        $stmt = $dbcon->prepare("SELECT * FROM comm_po WHERE po_director_approv ='FALSE' AND po_hod_approv='1'");
                        $stmt->execute();
                        $rowCount = $stmt->fetchAll();
                        $nRows = count($rowCount);
                            if ($nRows > 0) {
                        ...
                        case 'logistics':
                          require_once 'db.php';
                          $stmt = $dbcon->prepare("SELECT * FROM logistics WHERE trans_director_approv ='FALSE' AND trans_hod_approv='1'");
                          $stmt->execute();
                          $rowCount = $stmt->fetchAll();
                          $nRows = count($rowCount);
                              if ($nRows > 0) {
                          ...
                          case 'quotation_approval':
                            require_once 'db.php';
                            $stmt = $dbcon->prepare("SELECT * FROM quotation WHERE quotation_director_approv ='FALSE' AND quotation_hod_approv='1'");
                            $stmt->execute();
                            $rowCount = $stmt->fetchAll();
                            $nRows = count($rowCount);
                                if ($nRows > 0) {
...
function ViewApprovedRequest($view) {
  ...
  switch ($view) {
    ...
    case 'money_requisition':
      require_once 'db.php';
      $stmt = $dbcon->prepare("SELECT * FROM mr WHERE direct_approv='1' AND hod_approv='1'");
      $stmt->execute();
      $rowCount = $stmt->fetchAll();
      $nRows = count($rowCount);
          if ($nRows > 0) {
      ...
      case 'bills_proc':
        require_once 'db.php';
        $stmt = $dbcon->prepare("SELECT * FROM accounts_bl WHERE bl_director_approv='1' AND bl_hod_approv='1'");
        $stmt->execute();
        $rowCount = $stmt->fetchAll();
        $nRows = count($rowCount);
            if ($nRows > 0) {
        ...
        case 'vendor_payment':
          require_once 'db.php';
          $stmt = $dbcon->prepare("SELECT * FROM accounts_vp WHERE vp_director_approv='1' AND vp_hod_approv='1'");
          $stmt->execute();
          $rowCount = $stmt->fetchAll();
          $nRows = count($rowCount);
              if ($nRows > 0) {
          ...
          case 'daily_sales':
            require_once 'db.php';
            $stmt = $dbcon->prepare("SELECT * FROM sales_dscr WHERE dscr_director_approv ='1' AND dscr_hod_approv='1'");
            $stmt->execute();
            $rowCount = $stmt->fetchAll();
            $nRows = count($rowCount);
                if ($nRows > 0) {
            ...
            case 'transport':
              require_once 'db.php';
              $stmt = $dbcon->prepare("SELECT * FROM sales_transport WHERE transport_director_approv ='1' AND transport_hod_approv='1'");
              $stmt->execute();
              $rowCount = $stmt->fetchAll();
              $nRows = count($rowCount);
                  if ($nRows > 0) {
              ...
              case 'site':
                require_once 'db.php';
                $stmt = $dbcon->prepare("SELECT * FROM sales_site WHERE site_director_approv ='1' AND site_hod_approv='1'");
                $stmt->execute();
                $rowCount = $stmt->fetchAll();
                $nRows = count($rowCount);
                    if ($nRows > 0) {
                ...
                case 'recruitment':
                  require_once 'db.php';
                  $stmt = $dbcon->prepare("SELECT * FROM hr_rec WHERE rec_director_approv ='1' AND rec_hod_approv='1'");
                  $stmt->execute();
                  $rowCount = $stmt->fetchAll();
                  $nRows = count($rowCount);
                      if ($nRows > 0) {
                  ...
                  case 'leave':
                    require_once 'db.php';
                    $stmt = $dbcon->prepare("SELECT * FROM hr_lev WHERE lev_director_approv ='1' AND lev_hod_approv='1'");
                    $stmt->execute();
                    $rowCount = $stmt->fetchAll();
                    $nRows = count($rowCount);
                        if ($nRows > 0) {
                    ...
                    case 'vendor_registeration':
                      require_once 'db.php';
                      $stmt = $dbcon->prepare("SELECT * FROM ven_reg WHERE ven_director_approv ='1' AND ven_hod_approv='1'");
                      $stmt->execute();
                      $rowCount = $stmt->fetchAll();
                      $nRows = count($rowCount);
                          if ($nRows > 0) {
                      ...
                      case 'po_approvals':
                        require_once 'db.php';
                        $stmt = $dbcon->prepare("SELECT * FROM comm_po WHERE po_director_approv ='1' AND po_hod_approv='1'");
                        $stmt->execute();
                        $rowCount = $stmt->fetchAll();
                        $nRows = count($rowCount);
                            if ($nRows > 0) {
                        ...
                        case 'logistics':
                          require_once 'db.php';
                          $stmt = $dbcon->prepare("SELECT * FROM logistics WHERE trans_director_approv ='1' AND trans_hod_approv='1'");
                          $stmt->execute();
                          $rowCount = $stmt->fetchAll();
                          $nRows = count($rowCount);
                              if ($nRows > 0) {
                          ...
                          case 'quotation_approval':
                            require_once 'db.php';
                            $stmt = $dbcon->prepare("SELECT * FROM quotation WHERE quotation_director_approv ='1' AND quotation_hod_approv='1'");
                            $stmt->execute();
                            $rowCount = $stmt->fetchAll();
                            $nRows = count($rowCount);
                                if ($nRows > 0) {
